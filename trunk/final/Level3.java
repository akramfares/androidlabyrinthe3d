...
public class Level3 extends SimpleApplication implements ActionListener,PhysicsCollisionListener{
	private static Level3 app;
	...
	private void moveObstacle(float tpf) {
        if(sphereGroup.getNode().getLocalTranslation().z< -4f && speedSphere<0) speedSphere = 3f;
		if(sphereGroup.getNode().getLocalTranslation().z> 4f && speedSphere>0) speedSphere = -3f;
		sphereGroup.getNode().move(new Vector3f(0,0,speedSphere*tpf));
	}
	...
	public void onAction(String binding, boolean value, float tpf) {
		if (binding.equals("Left")) {
			...
			down = value;
				...
				player.setPhysicsLocation(new Vector3f(15, 25f, 0));
